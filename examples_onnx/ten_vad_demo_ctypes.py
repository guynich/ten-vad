...
#
"""
TEN VAD Python demo using ctypes
Drop-in replacement for ten_vad_demo.py that uses ctypes instead of pybind11.
...
# Import the ctypes VAD implementation
sys.path.insert(0, os.path.dirname(__file__))
import ten_vad_ctypes


def main():
    parser = argparse.ArgumentParser(description="TEN VAD Python Demo (ctypes)")
    ...
    # Create VAD instance (ctypes version)
    print(f"Using threshold:   {args.threshold}")
    print("Using ctypes implementation")

    try:
        vad = ten_vad_ctypes.VAD(hop_size=args.hop_size, threshold=args.threshold)
    except Exception as e:
